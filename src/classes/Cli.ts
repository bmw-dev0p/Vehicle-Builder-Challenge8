    ...
    performActions(): void {
        inquirer
            ...
            .then((answers) => {
                    ...
                    for (let i = 0; i < this.vehicles.length; i++) {
                        if (this.vehicles[i].vin === this.selectedVehicleVin) {
                            if (this.vehicles[i] instanceof Truck) { // since Truck is a class, we can use instanceof, not typeof
                                if (!(this.vehicles[i] as Truck).isTowing) { // check if truck is not already towing
                                ...
                                } else { // if truck is already towing
                                    console.log(`Sorry, ${this.vehicles[i].make} ${this.vehicles[i].model} is already towing a vehicle`);
                                }
                            } else { // if vehicle is not a truck
                ...
                //added functionality to stop towing
                // several ways to implement this, I chose to just use a boolean to track if truck is towing
                // option 2 is to add a method to stopTow, but that would require a vehicle parameter
                // option 3 is to modify the tow method to accept a boolean, but the interaction with findVehicleToTow would be more complex
                else if (answers.action === 'Stop Towing') {
                    for (let i = 0; i < this.vehicles.length; i++) {
                        if (this.vehicles[i].vin === this.selectedVehicleVin) { //select current vehicle
                            if (this.vehicles[i] instanceof Truck) { // check if Truck
                                if ((this.vehicles[i] as Truck).isTowing) { // check if truck is already towing
                                    console.log(`The ${this.vehicles[i].make} ${this.vehicles[i].model} is no longer towing`); //log
                                    (this.vehicles[i] as Truck).isTowing = false; // set isTowing to false
                                    // (this.vehicles[i] as Truck).stopTow(this.vehicles[i]); // call stopTow method
                                } else { // if truck is not towing
                                    console.log("Sorry, need to start towing before stopping");
                                }
                            } else { // if vehicle is not a truck
                                console.log("Sorry, selected vehicle is not a truck and cannot tow");
                            }
